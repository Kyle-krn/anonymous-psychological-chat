...
def next_companion(user_id):
    '''Поиск следующего собесендика'''
    user = get_user_by_id(user_id)
    db.users.update_one({'user_id': user['user_id']}, {'$set': {'last_companion_id': user['companion_id'], 'companion_id': None}})
    db.users.update_one({'user_id': user['companion_id']}, {'$set': {'last_companion_id': user['user_id'], 'search_companion': False, 'companion_id': None}})
...
def push_shadowing_message(message):
    ...
    if len(temp_message) <= 100:
        push_value(user_id=user['user_id'], key='temp_message', value=(message.text or message.caption))
    else:
        temp_message.append((message.text or message.caption))
        temp_message = temp_message[1:]
        set_value(user_id=user['user_id'], key='temp_message', value=temp_message)


def get_favorite_chat(user_id, favorite_id):
    '''Достает избранные чаты'''
    db.users.find_one({'user_id': user_id, 'favorite_chat.user_id': favorite_id}, {'favorite_chat.$': 1})['favorite_chat'][0]


def stop_premium_chat_db(user_id, data):
    push_value(user_id=user_id, key='premium_dialog_time', value=data)       # Пушим историю о премиум диалоге 
    set_value(user_id=user_id, key='time_start_premium_dialog', value=None)  # Переводим юзеров в режим не активного премиум диалога ("time_start_premium_dialog" is None)
